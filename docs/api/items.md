...
`source_kind=mp` 浏览历史公众号语料时不加默认 7 天窗口；`rss/html/aihot` 浏览仍使用默认 7 天窗口。对于缺失 `published_at` 的记录，时间窗口使用 `created_at` 兜底，但响应不会把入库时间伪装成 `publishedAt`。搜索 `q` 继续跨全量归档。
...
点击文章筛选、来源、分类或搜索提交都会重置当前列表和 cursor。AI 动态页始终保留当前热点区；筛选无结果沿用“暂无资讯”，接口失败沿用“加载失败，请稍后重试”。
